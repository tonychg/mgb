...
static void op_decrement(struct sm83_core *cpu, u8 *reg)
{
	u8 result = *reg - 1;
...
static void op_call_nn(struct sm83_core *cpu)
{
	...
	nn_lsb = cpu->memory->load8(cpu, cpu->pc);
	++cpu->pc;
	nn_msb = cpu->memory->load8(cpu, cpu->pc);
	++cpu->pc;
...
static void op_stack_push(struct sm83_core *cpu, u8 *r1, u8 *r2)
{
	cpu->sp--;
	cpu->memory->write8(cpu, cpu->sp, *r1);
	cpu->sp--;
	cpu->memory->write8(cpu, cpu->sp, *r2);
}

static void op_stack_pop(struct sm83_core *cpu, u8 *r1, u8 *r2)
{
	*r2 = cpu->memory->load8(cpu, cpu->sp);
	cpu->sp++;
	*r1 = cpu->memory->load8(cpu, cpu->sp);
	cpu->sp++;
}

static void op_stack_pop_pc(struct sm83_core *cpu, u16 *pc)
{
	u16 result;
	u8 low, high;

	low = cpu->memory->load8(cpu, cpu->sp);
	cpu->sp++;
	high = cpu->memory->load8(cpu, cpu->sp);
	cpu->sp++;
	result = unsigned_16(low, high);
	*pc = result;
}
...
void sm83_isa_execute(struct sm83_core *cpu, u8 opcode)
{
	...
	case 0x02:
		// LD (BC),a
		cpu->memory->write8(cpu, BC(cpu), cpu->a);
		break;
	case 0x03:
		// INC BC
		INC_BC(cpu);
	...
	case 0x04:
		// Z 0 H -
		op_increment(cpu, &cpu->b);
		break;
	case 0x05:
		// Z 1 H -
		op_decrement(cpu, &cpu->b);
		break;
	case 0x06:
		// LD B,n
	...
	case 0x0C:
		// Z 0 H -
		op_increment(cpu, &cpu->c);
		break;
	case 0x0D:
		// Z 1 H -
		op_decrement(cpu, &cpu->c);
		break;
	case 0x0E:
		// LD c,n
	...
	case 0x12:
		// LD (DE),a
		cpu->memory->write8(cpu, DE(cpu), cpu->a);
		break;
	case 0x13:
		// INC de
		INC_DE(cpu);
	...
	case 0x14:
		// Z 0 H -
		op_increment(cpu, &cpu->d);
		break;
	case 0x15:
		// Z 1 H -
		op_decrement(cpu, &cpu->d);
		break;
	case 0x16:
		// LD D,n
	...
	case 0x1C:
		// Z 0 H -
		op_increment(cpu, &cpu->e);
		break;
	case 0x1D:
		// Z 1 H -
		op_decrement(cpu, &cpu->e);
		break;
	case 0x1E:
		// LD E,n
	...
	case 0x24:
		// INC h
		// Z 0 H -
		op_increment(cpu, &cpu->h);
		break;
	case 0x25:
		// DEC h
		// Z 1 H -
		op_decrement(cpu, &cpu->h);
		break;
	case 0x26:
		// LD h,n
	...
	case 0x2C:
		// INC l
		// Z 0 H -
		op_increment(cpu, &cpu->l);
		break;
	case 0x2D:
		// DEC l
		// Z 1 H -
		op_decrement(cpu, &cpu->l);
		break;
	case 0x2E:
		// LD L,n
	...
	case 0x3C:
		// INC A
		op_increment(cpu, &cpu->a);
		break;
	case 0x3D:
		// DEC A
		op_decrement(cpu, &cpu->a);
		break;
	case 0x3E:
		// LD A,n
	...
	case 0xB6:
		// OR A,[HL]
		op_or(cpu, cpu->memory->load8(cpu, HL(cpu)));
		break;
	case 0xB7:
		// OR A,A
		op_or(cpu, cpu->a);
	...
	case 0xBE:
		// CP A,[HL]
		op_cp(cpu, cpu->memory->load8(cpu, HL(cpu)));
		break;
	case 0xBF:
		// CP A,A
		cpu_flag_clear(cpu);
	...
	case 0xC0:
		// RET NZ
		if (!cpu_flag_is_set(cpu, FLAG_Z)) {
			op_stack_pop_pc(cpu, &cpu->pc);
			cpu->cycles = 5;
		}
		break;
	case 0xC1:
		// POP BC
		op_stack_pop(cpu, &cpu->b, &cpu->c);
		break;
	case 0xC2:
		// JP NZ,nn
		if (!cpu_flag_is_set(cpu, FLAG_Z)) {
	...
	case 0xF1:
		// POP AF
		op_stack_pop(cpu, &cpu->a, &cpu->f);
		cpu->f &= 0xF0;
		break;
	case 0xF2:
		// LD A,(C)
		cpu->a = cpu->memory->load8(cpu, (u16)(0xFF00 + cpu->c));
		break;
	case 0xF3:
		// DI
		cpu->ime = false;
